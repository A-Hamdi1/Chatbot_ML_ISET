...
const SelfLearningPanel = () => {
  ...
  const fetchStatus = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/self-learning/status`
      );
  ...
  const fetchCandidates = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/self-learning/candidates`
      );
  ...
  const handleIntegration = async () => {
    ...
    try {
      ...
      const selectedQuestions = candidates.filter(
        (_, index) => selectedCandidates[index]
      );
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/self-learning/integrate`,
        {
  ...
  return (
    <Card sx={{ mt: 4 }}>
      <CardContent>
        ...
        >
          {loading ? (
            ...
            "Analyser les questions candidates"
          )}
        </Button>
        ...
        >
          Ce processus analyse les questions bien notées et propose leur
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Validation des questions candidates</DialogTitle>
        <DialogContent>
          {candidates.length === 0 ? (
            <DialogContentText>
              Aucune question candidate disponible actuellement. Les questions
              pourraient déjà être présentes dans la base de données ou il n'y a
              pas assez de questions bien notées.
            </DialogContentText>
          ) : (
            <>
              <DialogContentText sx={{ mb: 2 }}>
                Veuillez vérifier et valider les questions suivantes avant leur
                intégration dans la base de connaissances du chatbot.
              </DialogContentText>
              ...
              <List sx={{ width: "100%" }}>
                {candidates.map((candidate, index) => (
                  <Card
                    ...
                    sx={{
                      mb: 2,
                      border: selectedCandidates[index]
                        ? "1px solid #4caf50"
                        : "1px solid #e0e0e0",
                    }}
                  >
                    <CardContent>
                      ...
                      <Box sx={{ mt: 2 }}>
                        <TextField
                          ...
                          onChange={(e) =>
                            handleAnswerEdit(index, e.target.value)
                          }
                        ...
                        <Chip
                          ...
                          sx={{
                            ml: 1,
                            backgroundColor:
                              categoryColors[candidate.category] || "#757575",
                            color: "white",
                          }}
                        />
                      </Box>
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              loading ||
              candidates.length === 0 ||
              Object.values(selectedCandidates).every((val) => !val)
            }
      ...
      <Dialog open={resultOpen} onClose={() => setResultOpen(false)}>
        <DialogTitle>Résultat de l'intégration</DialogTitle>
        <DialogContent>
          {integrationResult && (
            <Alert
              severity={
