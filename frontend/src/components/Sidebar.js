  ...
  return (
    <>
      <Drawer
        ...
        open={open}
        sx={{
          width: open ? 280 : 0, // Largeur conditionnelle
          flexShrink: 0,
          "& .MuiDrawer-paper": {
            width: 280,
      ...
      >
        <Typography
          variant="h6"
          sx={{ p: 3, fontWeight: 500, color: "text.primary" }}
        >
          Chatbot ISET
        </Typography>
        <Divider />
        ...
        <Typography
          variant="subtitle1"
          sx={{ p: 2, fontWeight: 500, color: "text.primary" }}
        >
          Historique de Chat
        </Typography>
        <List>
          {sessions.map((session) => (
            ...
            >
              <ListItemButton component="a" href={`/?session_id=${session.id}`}>
                <ListItemText
                  primary={`Chat du ${new Date(session.date).toLocaleDateString(
                    "fr-FR"
                  )}`}
